...
timestamp=$(date +%s)
exec > >(tee -ia raspi-install-log-$timestamp.log)
exec 2> >(tee -ia raspi-install-log-$timestamp.log >&2)
...
if [ "$CUR_DIR" != "$GATEWAY_DIR" ]; then
    ...
    echo -e "${Y}cd installers/raspi-setup${NC}"
